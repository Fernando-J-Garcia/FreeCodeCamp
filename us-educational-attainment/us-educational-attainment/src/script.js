...
fetch('https://cdn.freecodecamp.org/testable-projects-fcc/data/choropleth_map/for_user_education.json').then(response=>response.json()).then(educationData=>{
...
{
   fetch('https://cdn.freecodecamp.org/testable-projects-fcc/data/choropleth_map/counties.json').then(response=>response.json()).then(usData=>{
    console.log(usData)
...
const drawMap=(educationData, usData)=>{
  ...
  countyData.forEach(d=>{
    educationData.filter((e,i)=>{
      let key = i;
      if(e.fips === d.id){
        idToStateMapper[d.id] = {'state':e.state,'area_name':e.area_name, 'bachelorsOrHigher':e.bachelorsOrHigher}
      }
    })
  });
  ...
  .append('svg').attr('id','canvas')
  .attr('width',width)
  .attr('height',height)
  
  const legendValues = [];  
  //split the education data into 8 pieces and make a color boxes with each value
  for(let i = edMin; i < edMax; i += Math.round(((edMax/8)+Number.EPSILON) * 100)/100)   {
    legendValues.push(i);
  }
  ...
  .append('g')
  .attr('transform',(d,i)=>`translate(${(width-(legendValues.length*50))+legendRectWidth*i}, 50)`)
  legend.append('rect')
  ...
  .on('mouseover',(event,d)=>{
    ...
    tooltip.text(county + ', ' + state + ' ' + bachelorsPercentage + '%')
