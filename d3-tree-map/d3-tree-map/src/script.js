fetch('https://cdn.freecodecamp.org/testable-projects-fcc/data/tree_map/movie-data.json').then(response=>response.json()).then(data=>{
  console.log(data)
  drawTreeMap(data);
})
...
const drawTreeMap=(data)=>{
  ...
  .append('g')
  .attr('transform',(d,i)=>`translate(${0}, ${(30*i)+legendPadding.top})`)
  legend.append('rect')
  ...
  const cellsContainer = svg.append('g')
  ...
  const cell = cellsContainer.selectAll('g')
